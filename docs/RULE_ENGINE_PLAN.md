...
Live evaluation uses **SQLite geofences** + **`presence_store` haversine** (same math as participant status), not the in-memory UTM `Geofence` class. Sunset uses **`astral`** + `settings_location` from the bundle.
...
1. **People vs actuators share a type hierarchy** — plain `Device` is for location participants (phones, tags); subclasses add `turn_on`, `open`, etc. Presence participants are *not* discovered on the LAN; their coordinates arrive via API.
...
Participants are **not** LAN devices. They are logical identities located by external pushes.
...
| --- | --- | --- |
| `participants_inside_geofence` | `geofence_id`, `participant_ids[]` | Every listed participant has a **fresh** fix and is inside the geofence (AND). Replaces my-tracks `CONDITION_ALL_INSIDE`. |
| `participants_outside_geofence` | `geofence_id`, `participant_ids[]` | Every listed participant is outside or stale/unknown (AND). Replaces my-tracks `CONDITION_ALL_OUTSIDE`. |
...
**Staleness alignment:** my-tracks treats “no location on record” as `unknown` → outside for `all_outside` rules. domesti-bot treats missing or stale fixes as **not inside** for `participants_inside_geofence` (equivalent for the arrival scenario).
...
| --- | --- |
| `CONDITION_ALL_INSIDE` + waypoint | `participants_inside_geofence` condition |
| `CONDITION_ALL_OUTSIDE` + waypoint | `participants_outside_geofence` condition (v1 — see conditions table) |
...
| --- | --- | --- | --- |
| `participant_id` | string | yes | Stable slug; matches my-tracks `User.username` |
| `lat` | float | yes | WGS84 |
| `lon` | float | yes | WGS84 |
...
- `404` — unknown `participant_id` (when auto-register is off).
- `422` — out-of-range lat/lon or malformed timestamp.

**Side effect:** update `PresenceRegistry`, persist last fix, `asyncio.create_task(evaluator.request_evaluation(reason="presence"))`.
...
Same body fields as the webhook (except `participant_id` comes from the path). Same responses.
...
Body: `{ "updates": [ { "participant_id", "lat", "lon", ... }, ... ] }` → `204`.
...
- `participants[]` — id, label, last fix, age, inside_geofences[]
...
1. **Startup** (in FastAPI lifespan, after discovery succeeds or in parallel — evaluator can idle until `device_state` exists):
   - Load geofences, participants, rules from SQLite into `RuleEngine`.
...
| Participant markers | Status tab / geofence tab: last fix per participant as small dots (distinct color from geofence circles) |
...
- `web/src/types.ts` — `RuleOut`, `GeofenceOut`, `ParticipantOut`, `RulesStatusOut`, … aligned with planned Pydantic models.
...
- Rules hub dialog: **Status**, **Rules**, **Participants** tabs wired to mock data source.
- Rule list + enable toggle + **rule editor** form (participants, geofence select, after-sunset, device action rows).
...
- **Geofences** tab: OSM tiles, circle overlays, draw toolbar, table, participant dots from mock status.
...
- `GET /v1/rules/status` — `RulesStatusOut` wire shape (participants, geofences, per-rule condition rows with `met`/`label`/`detail`, sun row).
...
| Condition eval | Both inside / one outside / stale participant / before vs after sunset (mock clock) |
...
- **Do not log raw coordinates at INFO** — DEBUG only, or log participant id + age.
- **Rate-limit** presence ingest (optional v1.1): max N updates per participant per minute to prevent flapping.
...
7. Desktop Status tab shows last fire time and current participant positions.

---

## Future extensions (out of scope for initial mainline)

- `edge_false` trigger variant (explicit departure edge); `participants_outside_geofence` condition ships in v1 but departure **actions** may follow later.
...
| Auto-register unknown participant IDs on ingest? | **No** by default; opt-in env flag. |
...
1. my-tracks relays locations via `POST /v1/webhooks/presence` on each GPS fix.
2. Henrique + Kristen + house + after-sunset fires once on arrival and actuates real Kasa + Tailwind devices.
3. Rules, geofences, and participants survive server restart (SQLite); UI uses `HttpRulesDataSource`.
