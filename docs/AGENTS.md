...
- **Optimistic UI updates (per-tile actions).** Clicks on a tile's `Turn it on/off` or `Open it / Close it` button mutate the controller's cached `state` *before* the HTTP round-trip via `predictDeviceState(family_id, device_id, next_state)` and re-render. The button label flips immediately so the user can chain actions without waiting for the response. The action's HTTP success is the confirmation; the continuous state watcher's next poll (≤ `DEFAULT_POLL_INTERVAL_S`) reconciles with the canonical reading. On action failure we fall back to `refresh()`, which either picks up the real state or flips the family frame red if the backend is unreachable. Bulk actions (`Turn off all`, `Close all`, `Turn everything off`) do *not* use optimistic updates: they target many devices, the per-device outcome isn't carried in the response, and the explicit `refresh()` after them gives the user a clean snapshot.
