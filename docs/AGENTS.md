...
- **Optimistic UI updates (bulk actions).** The per-family `Turn off all` / `Close all` and the global `Turn everything off` buttons use the same prediction machinery via `predictBulkOffForFamily(family_id)` / `predictBulkOffGlobal()`. Both helpers walk the cached state, skip every device with `exclude_from_global=true` (the backend won't touch them, so neither should the overlay), and call `predictDeviceState(..., bulkOffStateForKind(device.kind))` for each remaining device — `"off"` for switches, `"closed"` for doors. The grace window then suppresses any contradicting poll reading until the bulk action is confirmed or the 8s window expires. On bulk-dispatch failure we clear the just-registered predictions (family-scoped for `onBulkOffFamily`, everything for `onBulkOffGlobal`) so a request that never landed server-side doesn't leave the UI lying about device state.
- **Standardized state + action colors.** Two CSS variables drive every state / action color in the landing page so the rule stays consistent across both device kinds and both control surfaces: `--accent` (green) for active / positive outcomes and `--danger` (red) for inactive / destructive outcomes (`--pending` amber stays reserved for the transient `unknown` door state). **State badges**: `tile-state-on` and `tile-state-open` are green; `tile-state-off` and `tile-state-closed` are red; `tile-state-unknown` is amber. **Per-tile toggle**: colored by the *action* it will perform, not the current state — `data-on="false"` (next click turns the device on or opens it) is green, `data-on="true"` (next click turns it off or closes it) is red. **Bulk buttons**: per-family `Turn off all` / `Close all` and global `Turn everything off` all use `btn-danger` (red) because every bulk path in the app is destructive. When introducing a new button, pick the variable that matches the *outcome* of clicking it; do not reach for `--family-color` (that variable owns the family-grouping cues — the left tile stripe and the swatch next to the family heading — and must not double-duty as state).lure (HTTP throw) we drop the prediction immediately and refresh — the action provably didn't land, so the grace window would only mislead. Bulk actions (`Turn off all`, `Close all`, `Turn everything off`) clear all in-scope pending predictions before dispatch — their canonical refresh should win immediately rather than be held by stale per-tile guesses.
- **Disable actions when backend is unreachable.** When `refresh()` fails and `connected` flips to `false`, every action element is rendered with the HTML `disabled` attribute: per-tile toggle, per-tile "exclude" checkbox, per-family bulk button, and the global "Turn everything off" button. CSS rules (`.tile-toggle:disabled, .btn:disabled`) gray-scale them and set `cursor: not-allowed`; the red family frame plus the grayed controls together make the offline state unmistakable. As soon as the next poll succeeds the controls re-enable on the next render — no extra plumbing needed.
