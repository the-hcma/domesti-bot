...
│   ├── sonos_device_manager.py            Sonos speakers (soco)
...
- **`Cache-Control: no-store` on `/v1/settings`**: `_SettingsCacheControlMiddleware` in `app/api/app.py` stamps `Cache-Control: no-store` on **every** response whose path is `/v1/settings` or under `/v1/settings/` (all four settings routers), success and error alike. Several settings GETs return secret material on read (`GET /v1/settings/tailwind-token`, `/v1/settings/kasa-credentials`, `/v1/settings/ep1-noise-psk`); `no-store` keeps a shared / proxied browser cache from retaining those payloads after the API key or the stored value changes. New settings sub-routers inherit this automatically as long as they keep the `/v1/settings` prefix.
...
- **Deferred-discovery lifespan (HTTP server is ready before discovery finishes).** The FastAPI lifespan in `app.api.app.create_app` is non-blocking:
  ...
  - **CLI REPL** mirrors the same contract: `scripts/domesti-bot` shows the prompt as soon as process wiring is up. Family discovery runs in the background; ready lines print as they land. Device commands while a family is still sweeping print a discovery-in-progress error (not a traceback). Tab completion uses `preferred_label (mac)` via `format_device_display` (still accepts typed MAC, label, or the combined string). After Kasa discovery succeeds, the **CLI** one-shot (`sync_kasa_vendor_aliases=True` on `bootstrap_device_managers`) pushes unique preferred labels onto vendor aliases. HTTP `_run_discovery` leaves that flag false so server (re)starts do not rewrite hardware names. `GET /v1/completion-aliases` returns `{display, matches}` objects per family (breaking vs the old `list[str]` lists; the remote CLI still accepts both).
...
- **Sonos speakers in the web UI.** The `sonos` family renders alongside `kasa` and `tailwind` in alphabetical order. `UIDeviceOut.kind = "speaker"` and `state = "playing" | "paused" | "unknown"`. Per-tile clicks call `POST /v1/ui/sonos/zones/{uid}/toggle` (body: `{"playing": bool}`) which dispatches to `SonosSpeakerDevice.pause()` / `resume()`; the per-family `Pause all` button calls `POST /v1/ui/sonos/pause-all` (skips speakers whose cached `is_playing` is not `True`); and the global `Turn off / pause / close everything` action calls `_bulk_pause_sonos_apply_impl` after the kasa and before the tailwind step. The `unknown` state covers the pre-poll window plus any speaker whose UPnP transport read failed; the controller treats it the same way it treats `paused` for the toggle (so the next click pauses — a SoCo `pause` on a stopped queue is a no-op rather than an error). Read state from `SonosSpeakerDevice.is_playing` (`bool | None` cache), never from `transport_state_summary()` on the request path — the latter is a live UPnP call and would block the HTTP worker on a slow speaker. The cache is refreshed by `SonosPollingWatcher` every `DEFAULT_POLL_INTERVAL_S` seconds and by every `pause()` / `resume()` call (post-action sync). Family color: `#8B5CF6` (violet) — picked so kasa-blue, sonos-violet, and tailwind-green are all distinguishable on a single screen.
- **Sonos UPnP 701 ("Transition not available").** Sonos rejects a `Play` on a speaker with an empty queue / no media source, and rejects either `Play` or `Pause` while a speaker is mid-`TRANSITIONING`. `SonosSpeakerDevice.pause()` / `resume()` catch the resulting `SoCoUPnPException(error_code="701")`, refresh the cached `is_playing` from a live UPnP read (so the next render shows truth), and re-raise as `SonosTransitionUnavailableError`. The HTTP endpoint maps that domain error to **`409 Conflict`** with a human-readable detail message (`"… cannot resume — the queue is empty or the speaker is mid-transition. Pick something to play from the Sonos app first."`); the front-end surfaces that detail through the action-error toast (see below) and then runs the standard `clearPendingPrediction → refresh()` recovery so the tile snaps to reality. `_bulk_pause_sonos_apply_impl` catches the same domain error and silently drops the offending speaker from both `affected` and `skipped` (it isn't excluded — the user didn't ask — and it wasn't paused either), so one stuck speaker never takes down a global "Turn off / pause / close everything" action. Other UPnP fault codes propagate as-is — only 701 has a known graceful translation.
- **Family header icons.** Each family-section renders an inline SVG icon next to its `<h3>` title — lightbulb for `kasa` ("Lights & plugs"), speaker for `sonos` ("Sonos speakers"), pitched-roof house for `tailwind` ("Garage doors"). Icons live in `FAMILY_ICON_PATHS` in `web/src/main.ts` as plain SVG `d=` strings; `createFamilyIcon(family_id)` builds the `<svg>` in the SVG namespace (`createElementNS` — `createElement` would produce inert HTMLUnknownElement nodes), wraps the paths in `stroke="currentColor"` so the CSS rule `.family-icon { color: var(--family-color, var(--muted)); }` paints them per-family, and stamps `aria-hidden="true"` because the adjacent `<h3>` text is the real label for assistive tech. Adding a new family is a one-line entry here plus the existing `color` field in `app.api.ui_state._FAMILIES`. The icons replaced an earlier 10×10 `::before` colored square — the family colour cue stayed (now carried by the SVG fill instead of a swatch), and the icon adds a glanceable category hint.
- **Action / status toast colors.** User-visible toasts and Settings status lines for completed actions must be unambiguously colored: **green** (`ToastVariant.Success` / `--accent` / `.action-toast-success` / `.settings-dialog-status-success`) on success, **red** (`ToastVariant.Error` / `--danger` / `.action-toast-error` / `.settings-dialog-status-error`) on failure. Prefer `showSuccessToast` / `showErrorToast` / `showInfoToast` from `web/src/ui-toast.ts` and `setSettingsDialogStatus` from `web/src/settings-status.ts`. Use `ToastVariant.Info` only for in-progress copy or non-outcome hints — never for a finished success or failure. Keep `.cursor/rules/toast-success-failure-colors.mdc` aligned with this section.
- **Action-error toast for recoverable failures.** Tile / bulk feedback and Sonos 409 hints go through `showErrorToast` / `showSuccessToast` / `showInfoToast` in `web/src/ui-toast.ts` (mounted on `document.body` so the background poll's `render()` — which calls `replaceChildren()` on `#app` — never blows them away). One toast at a time: a subsequent toast replaces the current one and resets the auto-dismiss timer; the user can also click the `×` button. Wired from `onToggleDevice` → `onSpeakerTileActionFailure` when `err instanceof HttpError && err.status === 409` — currently the only per-tile path with a user-actionable server-side hint. `HttpError.detail` (parsed from FastAPI's `{"detail": "…"}` body at construction time, memoised) carries the message. Distinct from `renderError`, which is **destructive** (replaces all of `#app`) and reserved for fatal/bootstrap failures. Reuse `showErrorToast` for any future endpoint that returns 4xx with a hint the user can act on; non-actionable errors should keep falling back to `console.warn` + `refresh()`.
...
- **GoTailwind token (encrypted SQLite).** The six-digit Local Control Key can be stored in `app_secrets` (Fernet ciphertext) instead of plain environment variables. Resolution order for the token: CLI `--tailwind-token` → `TAILWIND_TOKEN` env → decrypted row in SQLite (`app.tailwind_credentials.resolve_tailwind_token`). The Fernet master key loads from `DOMESTI_BOT_SECRETS_KEY` or from gitignored `domesti-bot.config.json` at the repo root (`domesti_secrets_key` field — see `domesti-bot.config.json.example`). Override the file path with `DOMESTI_BOT_CONFIG_FILE`. **`setup-secrets` REPL command** — interactive helper in `app.domesti_bot_cli` that generates or accepts a Fernet key and writes `domesti-bot.config.json` (mode `0600`). **Desktop web UI** — ☰ menu → Settings (`PUT /v1/settings/tailwind-token`); token is never returned on read (`GET /v1/settings/tailwind-token` reports status only). After saving, restart the server so discovery reloads the token. **Hub hardware info** — `GET /v1/settings/tailwind/hub-info` reports the controller identity captured on the last successful connect (`product` / model, `firmware_version`, `protocol_version`, `device_id`, `hub_mac`, `host`, `number_of_doors`) from the live `GotailwindDeviceManager` (`_HubMetadata`, populated in `fetch()` from `Tailwind.status()`); it returns `reachable: false` with null fields when GoTailwind was skipped or discovery has not completed. Settings → GoTailwind renders it as a read-only block above the token form.
- **Settings credential Test (read-only).** Each auth card (Tailwind, Kasa, My Tracks, Vizio, EP1) has a **Test** button that calls `POST /v1/settings/…/test` (Vizio: `…/tvs/{device_id}/auth/test`). Probes use ephemeral clients only (`app.settings_credentials_test`) — they never call `rediscover` / `disconnect` on live managers. Credential failures return **200** with `ok: false` and a `detail` string; **4xx** only when the test cannot run (no credentials, no KLAP hosts, missing My Tracks password/domain, unknown Vizio `device_id`, or EP1 with neither PSK nor resolvable host). Optional form overrides (token / username+password / host / Noise PSK) win over stored CLI/env/database resolution. EP1 Homey firmware is plaintext — Test with a host and empty PSK is valid. Operator bring-up: [`docs/EP1.md`](EP1.md).
...
- **Remote timeouts and bounded retries:** `.cursor/rules/remote-timeouts-retries.mdc`
  (`alwaysApply`). Every network call (My Tracks `httpx`, Vizio SmartCast `aiohttp`,
  Tailwind HTTP, Sonos UPnP, python-kasa, pychromecast, outbound SMTP, `gh`/`curl`)
  sets an explicit timeout — reuse the module constant (`_REQUEST_TIMEOUT_S`,
  `_DEFAULT_REQUEST_TIMEOUT` / `_POLL_REQUEST_TIMEOUT`) — and any retry is bounded
  (cap or budget, backoff, `Retry-After`), transient-only, and never re-sends a
  non-idempotent write. Org template sync: [repository-helpers#570](https://github.com/the-hcma/repository-helpers/issues/570).
...
**ARP as liveness.** Explicit `refresh-discovery` / `rediscover()` drops a cached MAC only when the family protocol **and** the local ARP/neighbor table both miss it. A MAC still visible on the LAN is kept; `UIDeviceOut.unresponsive` is true and the tile/CLI copy is :data:`app.device_display.DEVICE_UNRESPONSIVE_ON_LAN` (`"On the network, but might be unresponsive"`). Cache-first startup still reconnects from SQLite without requiring ARP.

**Human-visible device labels.** Operator-facing copy uses `preferred_label (mac)` via :func:`app.device_display.format_device_display` (and the TypeScript mirror `formatDeviceDisplay` / `resolveDeviceLabel` in `web/src/rule-summary.ts`). Surfaces include emails, UI copy, rule condition status `detail`, operator-facing log prose (`detail=` / action outcomes), validation/warning messages, and Automations summaries. Resolve the name argument as: live `preferred_label` (`lookup_preferred_label` in `app.rule_actions`) when device managers / `device_state` are available; else optional rule/JSON snapshot `display_name` on device refs; else bare `device_id`. Store, wire, and compare as raw MAC `device_id` only — never put formatted strings into ids, keys, or equality checks. Structured log keys may keep raw `device_id=…`; human `detail=` / message prose must call `format_device_display`. Optional snapshot `display_name` is non-authoritative; stale vs live preferred labels are a Warning (`stale_device_display_name`), not Broken. Keep `.cursor/rules/device-display-name-mac.mdc` aligned with this section.
...
| **EP1** | `ep1_known_devices` (host, port, **mac**, friendly_name) | Reconnect each cached host over the ESPHome native API (port **6053**). Homey / pre-adoption firmware is **plaintext** (optional Noise PSK). Empty cache / `--force-discovery` with no `--ep1-host`: browse `_esphomelib._tcp` for Everything Presence One (`discover_ep1_hosts`). | Normalized MAC from `device_info`; skip if unknown |
...
**`last_discovery_source` reporting**. Each LAN-discovering manager (`KasaDeviceManager`, `SonosDeviceManager`, `AndroidTvDeviceManager`, `Ep1DeviceManager`) MUST set `self._last_discovery_source` to `"cache"` or `"discovery"` at the end of `fetch()` and expose it via a `last_discovery_source: str | None` property. The semantics are precise — they directly drive the per-backend "ready" line that the user reads to decide if a slow start is suspicious:
