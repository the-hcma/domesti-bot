...
- **`pytest`** is the framework. Configuration lives in the `[tool.pytest.ini_options]` block of `pyproject.toml`:
  ...
  - **Hang guards** — `pytest-timeout` enforces a **60s per-test** limit (`timeout_method = thread`, compatible with xdist). Prefer `await asyncio.Event().wait()` (cancel-friendly forever-block) over long `asyncio.sleep(N)` in cancel/shutdown tests, and wrap synchronization `Event.wait()` calls the test itself awaits with `asyncio.wait_for(..., timeout=…)`. CI’s hermetic job also has `timeout-minutes: 15` as a job-level backstop. Override locally with `--timeout=0` under `pdb`.
