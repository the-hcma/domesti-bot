...
async def test_coordinator_skips_when_wifi_home_bssid_matches(
    ...
    db = tmp_path / "discovery.sqlite"
    _write_edge_rule(bundle)
    _seed_db(db, home_wifi_ssid=None)
    monkeypatch.setenv("DOMESTI_AUTOMATION_RULES_FILE", str(bundle))
...
@pytest.mark.asyncio
async def test_coordinator_skips_when_blank_home_ssid_falls_back_to_bssid(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Blank stored home SSID is treated as absent so BSSID fallback still works."""
    bundle = tmp_path / "rules.json"
    db = tmp_path / "discovery.sqlite"
    _write_edge_rule(bundle)
    _seed_db(db, home_wifi_ssid="")
    monkeypatch.setenv("DOMESTI_AUTOMATION_RULES_FILE", str(bundle))
