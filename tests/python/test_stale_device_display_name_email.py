...
def test_maybe_send_is_atomic_across_threads(tmp_path: Path) -> None:
    cache = tmp_path / "cache.sqlite"
    bootstrap_schema(cache)
    vacation = VacationModeSettingsOut(
        enabled=True,
    ...
    def _deliver(*_args: object, **_kwargs: object) -> SmtpDeliveryResult:
...
def test_maybe_send_skips_when_already_sent_today(tmp_path: Path) -> None:
    cache = tmp_path / "cache.sqlite"
    bootstrap_schema(cache)
    assert upsert_operator_digest_last_sent_at(
        cache,
        digest_id=OperatorDigestId.STALE_DEVICE_DISPLAY_NAME,
