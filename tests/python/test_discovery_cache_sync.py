...
@pytest.mark.asyncio
async def test_maybe_sync_retries_when_cache_shrinks_after_failed_reload(
    tmp_path: Path,
) -> None:
    """Authoritative cache shrink must retry sync even when it matches the failed fingerprint."""
...
@pytest.mark.asyncio
async def test_maybe_sync_noop_when_klap_row_has_cached_mac(tmp_path: Path) -> None:
    """KLAP rows fingerprint by host only so preserved MACs do not cause drift."""
...
@pytest.mark.asyncio
async def test_maybe_sync_vizio_noop_when_live_has_env_token_tv_and_empty_table(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("DOMESTI_BOT_SECRETS_KEY", Fernet.generate_key().decode("ascii"))
    ...
    kitchen = MagicMock()
    kitchen.identifier = "00:bd:3e:d5:f0:11"
    den = MagicMock()
    den.identifier = "00:bd:3e:d5:f0:22"
    mgr = MagicMock(spec=VizioDeviceManager)
    mgr.tvs = (kitchen, den)
    mgr._cli_auth_token = None
    mgr._env_auth_token = "shared-env-token"
    mgr.reload_from_cache = AsyncMock()
