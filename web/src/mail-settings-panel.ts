// SMTP / mail settings panel (mock-backed; mirrors my-tracks admin email tab).
...
export async function mountMailSettingsPanel(
  ...
  lead.textContent =
    "Outgoing email for automation notifications. Send a successful test before saving (same flow as my-tracks).";
  ...
  fromInput.value = defaults.from_address;
  appendLabeledField(
    form,
  ...
  const readDraft = (): SmtpConfigIn => ({
    ...
    port: Number(portSelect.value) || 587,
  ...
  for (const el of [hostInput, portSelect, usernameInput, passwordInput, fromInput]) {
